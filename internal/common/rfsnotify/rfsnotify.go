...
//
// Vendored from github.com/dietsche/rfsnotify (MIT, see LICENSE beside this file).
// The upstream module pins gopkg.in/fsnotify.v1 v1.4.7, whose kqueue backend opens
// every watched file with O_EVTONLY but no O_CLOEXEC. With a few thousand org files
// under watch, every descriptor leaked into each child process we spawn (pdflatex,
// the editor, ...) and those children died with "too many open files". Newer
// github.com/fsnotify/fsnotify sets O_CLOEXEC, so the only change here is the import.
package rfsnotify
