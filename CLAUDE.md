...
- `internal/app/orgs/dnd.go` — the `/dnd/*` REST endpoints and the character build session store. `internal/app/orgs/plugs/dnd/` owns the shared ruleset library plus the `dndsheet`/`dndlatex`/`dndpdf` exporters.
...
2. Only the property drawer plus the equipment and spell tables are parsed back out of an org sheet. Every other section is regenerated by `RenderOrg`, so derived data must never be the only copy of something.
