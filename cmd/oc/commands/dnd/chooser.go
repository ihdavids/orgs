...
// first. The matcher itself lives in internal/common/dnd so that the html
// character sheet's item search filters a list exactly the way this does.
func fuzzyMatches(filter string, labels, names []string) []int {
	return dnd.FuzzyMatches(filter, labels, names)
}
